...
PROTOCOL = json.loads((Path(__file__).resolve().parents[1]/'configs/model.json').read_text())
...
def assert_runtime_compatibility():
    ...
    if versions['rdkit']!=PROTOCOL['rdkit_version']:
        raise RuntimeError('Benchmark response replay requires RDKit '+PROTOCOL['rdkit_version']+
